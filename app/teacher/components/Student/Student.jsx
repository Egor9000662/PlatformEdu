...
function Student({
	...
	const [unseenWeeks, setUnseenWeeks] = useState(0);
	const [isActive, setIsActive] = useState(false);
	const [isModalNextWeekVisible, setIsModalGroupNextWeekVisible] = useState(false);
	const [openAccess, setOpenAccess] = useState(false);

	let allSchedule = Object.values(schedule);
	let lastLessons = allSchedule.reduce((prev, cur) =>
		cur?.weekNumber > prev.weekNumber ? cur : prev
	);
	let lastLessonNumber = lastLessons.weekNumber;

	function isValidUnseenWeeks(unseenData) {
		const weeksId = Object.keys(unseenData);
		let unseenMessageCount = [];
		for (const weekId of weeksId) {
			if (weekId !== 'week-undefined') {
				unseenMessageCount.push(weekId);
			}
		}
	...
	useEffect(async () => {
		const unseenData = await hometaskChatStore.getUnseenWeekly(uid);
		if(unseenData) {
			let unseenMessageCount = isValidUnseenWeeks(unseenData);
			if (unseenMessageCount.length) {
				setUnseenWeeks((prev) => (prev = unseenMessageCount.length));
			}
		}
	}, []);
	...
	useEffect(() => {
		if (studentProgress) {
			setOpenAccess(studentProgress.openAccess && lastLessonNumber > currentWeek);
		}
	}, [studentProgress]);

	const openNextWeekAccess = () => async () => {
		setIsModalGroupNextWeekVisible(false);
		await setNextWeekAccess(uid, currentWeek);
		setIsActive(false);
	};

	const deleteNextWeekAccess = () => async () => {
		setIsModalGroupNextWeekVisible(false);
		await removeNextWeekAccess(uid);
		setIsActive(true);
	};

	const showModalNextWeek = (event) => {
		event.preventDefault();
		setIsModalGroupNextWeekVisible(true);
	};

	useEffect(() => {
		setIsModalNextWeekVisible(isModalNextWeekVisible);
	}, [isModalNextWeekVisible]);

	useEffect(() => {
		const isActive = !(nextWeek?.id && Number(nextWeek?.id?.split('-')[1]) > currentWeek);
		setIsActive(isActive);
	}, []);

	return (
		<div className="stdList-student">
			<div className="student-avatar">
				{isBestHometask && (
					<img
						className="student-crownIcon"
						src={Crown}
						alt="crown"
					/>
				)}
				<img
					className="student-avatar__img"
					src={avatarURL ? avatarURL : FrontendAvatar}
					alt="avatar"
				/>
			</div>
			<div className={isBest ? 'student student_best' : 'student'}>
				<div className="student-info">
					<span className="student-data">
						<div
							className={isBest ? 'std-name bestStd' : 'std-name'}
						>
							{name}
						</div>
						<div className="data">
					...
					<div className="std-weeks_info">
						{openAccess && (
							<div className="std-accessNextWeek">
								<button
									type="button"
									onClick={showModalNextWeek}
									title="Открыть доступ к следующему уроку"
								>
									<InboxOutlined
										className={
											isActive
								...
								<Modal
									...
									title={
										isActive
											? `Open access to next week № ${currentWeek + 1}`
											: `Close access`
									}
									open={isModalNextWeekVisible}
									onOk={() => setIsModalGroupNextWeekVisible(false)}
									onCancel={() => setIsModalGroupNextWeekVisible(false)}
									footer={[
										<Button
											key="submit"
											type="primary"
											onClick={
												isActive
													? openNextWeekAccess()
													: deleteNextWeekAccess()
											}
											className={isActive ? 'greenBtn' : 'redBtn'}
										>
											{isActive ? 'Open' : 'Close'}
										</Button>,
									]}
...
export default inject(({ oneUserStore, hometaskChatStore }) => {
	const { setNextWeekAccess, removeNextWeekAccess } = oneUserStore;
